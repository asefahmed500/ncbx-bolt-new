/*
  ...
  1. Database Constraints
    - Add validation constraints for website data
...
-- Create indexes for optimal performance (without CONCURRENTLY to avoid transaction block issues)
DO $$
BEGIN
  -- Check and create indexes only if they don't exist
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_user_id_idx') THEN
    CREATE INDEX websites_user_id_idx ON websites(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_status_idx') THEN
    CREATE INDEX websites_status_idx ON websites(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_created_at_idx') THEN
    CREATE INDEX websites_created_at_idx ON websites(created_at DESC);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_updated_at_idx') THEN
    CREATE INDEX websites_updated_at_idx ON websites(updated_at DESC);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_user_created_idx') THEN
    CREATE INDEX websites_user_created_idx ON websites(user_id, created_at DESC);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_user_status_idx') THEN
    CREATE INDEX websites_user_status_idx ON websites(user_id, status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_user_updated_idx') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'websites_search_idx') THEN
...
GRANT USAGE ON SEQUENCE websites_id_seq TO authenticated;
