/*
  ...
  1. New Features
    - Add user_role enum with 'user' and 'admin' values
...
-- Premium templates - admins can manage
...
-- Note: This will create the profile, but the auth.users entry needs to be created through Supabase Auth
DO $$
DECLARE
  admin_user_id uuid;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'admin@gmail.com') THEN
    ...
    RAISE NOTICE 'Please create the auth user manually in Supabase Auth with email: admin@gmail.com';
  ELSE
    -- Update existing admin to ensure they have admin role
    UPDATE profiles 
    SET role = 'admin', plan = 'business', updated_at = now()
    WHERE email = 'admin@gmail.com';
    
    RAISE NOTICE 'Admin profile updated: admin@gmail.com';
  END IF;
END $$;
...
-- Grant permission to log admin actions
...
BEGIN
  ...
  RAISE NOTICE '- Default admin email: admin@gmail.com';
  RAISE NOTICE 'Next step: Create auth user for admin@gmail.com with password admin123';
END $$;