/*
  # Final Database Setup for NCBX Website Builder
  ...
  3. Performance
    - Indexes for email, user_id, status, and created_at
...
-- Create enum type user_plan if not exists
DO $$
BEGIN
...
-- Create enum type website_status if not exists
DO $$
BEGIN
...
-- Drop existing triggers before recreating (to avoid errors)
